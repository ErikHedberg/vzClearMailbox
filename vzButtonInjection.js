...
if(!document.getElementById("deleteAll")) {
	...
	divMain.setAttribute("onClick","var deb = false function deletePage() {if(!deb && document.getElementById('INBOX_Emails').className) {deb = true;document.getElementById('selector').click();document.getElementById('delButton').click();document.getElementsByClassName('BtnRed')[2].click();}else{deb=false;return;}}sleepTime = 500;numPages = parseInt(document.getElementsByClassName('bold fl')[1].firstChild.nodeValue);interval = setInterval(function() {deletePage()},sleepTime);setTimeout(function() {deb=false;clearInterval(interval)},sleepTime*(2*numPages));");
